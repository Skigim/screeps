...
export class LegatusLegionum {
  ...
  private assignTask(creep: Creep, tasks: Task[]): void {
    ...
    if (suitableTasks.length === 0) {
      ...
      tasks.forEach(t => {
        ...
        if (t.type === 'PICKUP_ENERGY' && isSpecializedHarvester) reason.push('SPECIALIZED_HARVESTER');
   ...
   */
  private canDisplaceHarvester(_creep: Creep, task: Task, myWorkParts: number): boolean {
   ...
   */
  private displaceWeakestHarvester(creep: Creep, task: Task, myWorkParts: number): void {
