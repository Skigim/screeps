import { Task } from '../Task';

export const MIN_LIFETIME_FOR_BOOST = 0.9;

export type getBoostedTargetType = StructureLab;

function boostCounts(creep: Creep): { [boost: string]: number } {
    return _.countBy(creep.body, (bodyPart: BodyPartDefinition) => bodyPart.boost);
}

const boostParts: { [key: string]: BodyPartConstant } = {
    'UH': ATTACK,
    'UO': WORK,
    'KH': CARRY,
    'KO': RANGED_ATTACK,
    'LH': WORK,
    'LO': HEAL,
    'ZH': WORK,
    'ZO': MOVE,
    'GH': WORK,
    'GO': TOUGH,
    'UH2O': ATTACK,
    'UHO2': WORK,
    'KH2O': CARRY,
    'KHO2': RANGED_ATTACK,
    'LH2O': WORK,
    'LHO2': HEAL,
    'ZH2O': WORK,
    'ZHO2': MOVE,
    'GH2O': WORK,
    'GHO2': TOUGH,
    'XUH2O': ATTACK,
    'XUHO2': WORK,
    'XKH2O': CARRY,
    'XKHO2': RANGED_ATTACK,
    'XLH2O': WORK,
    'XLHO2': HEAL,
    'XZH2O': WORK,
    'XZHO2': MOVE,
    'XGH2O': WORK,
    'XGHO2': TOUGH,
};

export class TaskGetBoosted extends Task {
    public static taskName = 'getBoosted';

    public get target(): getBoostedTargetType | null {
        return super.target as getBoostedTargetType | null;
    }

    constructor(target: getBoostedTargetType, boostType: _ResourceConstantSansEnergy, partCount: number | undefined = undefined, options: TaskOptions = {}) {
        super(TaskGetBoosted.taskName, target, options);
        // Settings
        this.data.resourceType = boostType;
        this.data.amount = partCount;
    }

    isValidTask(): boolean {
        const lifetime = _.any(this.creep.body, (part: BodyPartDefinition) => part.type === CLAIM) ? CREEP_CLAIM_LIFE_TIME : CREEP_LIFE_TIME;
        if (this.creep.ticksToLive && this.creep.ticksToLive < MIN_LIFETIME_FOR_BOOST * lifetime) {
            return false; // timeout after this amount of lifespan has passed
        }
        const partCount = (this.data.amount || this.creep.getActiveBodyparts(boostParts[this.data.resourceType!]));
        return (boostCounts(this.creep)[this.data.resourceType!] || 0) < partCount;
    }

    isValidTarget(): boolean {
        return true; // Warning: this will block creep actions if the lab is left unsupplied of energy or minerals
    }

    work(): number {
        const partCount = (this.data.amount || this.creep.getActiveBodyparts(boostParts[this.data.resourceType!]));
        if (this.target!.mineralType === this.data.resourceType &&
            this.target!.mineralAmount >= LAB_BOOST_MINERAL * partCount &&
            this.target!.energy >= LAB_BOOST_ENERGY * partCount) {
            return this.target!.boostCreep(this.creep, this.data.amount);
        } else {
            return ERR_NOT_FOUND;
        }
    }
}
