...
function runHauler(creep) {
    ...
    if (!creep.memory.working) {
        ...
        if (isSourceAssignment && targetStructure) {
            ...
            pickupFromSource(creep, targetStructure);
        }
 ...
 */
function pickupFromSource(creep, source) {
    ...
    if (nearbyContainers.length > 0) {
        ...
        if (closest) {
            const result = creep.withdraw(closest, RESOURCE_ENERGY);
            if (result === ERR_NOT_IN_RANGE) {
                creep.travelTo(closest);
            }
            return;
 ...
 */
function pickupFromStructure(creep, structure) {
